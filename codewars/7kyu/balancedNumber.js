...
function balancedNum(number){
  ...
  if(number.length % 2 !== 0){
    num1 = number.slice(0, number.length / 2).reduce((a,b) => +a + +b, 0)
    num2 = number.slice(-number.length / 2).reduce((a,b) => +a + +b, 0)
  }else if(number.length % 2 == 0){
    num1 = number.slice(0, (number.length / 2) - 1).reduce((a,b) => +a + +b, 0)
    num2 = number.slice(-(number.length / 2) + 1).reduce((a,b) => +a + +b, 0)
  }
  return num1 == num2 ? "Balanced" : "Not Balanced"
}
